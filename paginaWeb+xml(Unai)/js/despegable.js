document.addEventListener('DOMContentLoaded', function () {
    ...
    temporadaButton.addEventListener('click', async function (e) {
        ...
        try {
            const response = await fetch('/xml'); // Obtener el archivo XML
            if (!response.ok) {
            ...
            // Declarar xmlFiles en un alcance más amplio
            const xmlFiles = []; // Declarar aquí para que esté disponible

            const regex = /<a\s+href="([^"]+\.xml)"/g; // Usa el regex que funciona
            let match;
            ...
            if (dropdownContent) {
                ...
                xmlFiles.forEach(file => {
                    ...
                    item.textContent = file; // Asignar el nombre del archivo como texto
    ...
    async function loadXMLContent(fileName) {
        try {
            const xmlResponse = await fetch(`${fileName}`); // Obtener el archivo XML
            if (!xmlResponse.ok) {
